...
export function registerMetadataCommands(context: MetadataCommandContext): void {
	...
	plugin.addCommand({
		...
		callback: async () => {
			...
			if (file.extension !== "md" && file.extension !== "docx") {
				new Notice(
					`⚠️ Unsupported file type: ${file.extension}. Use .md or .docx files.`
				);
				return;
			}
	...
	plugin.addCommand({
		...
		checkCallback: (checking: boolean) => {
			...
			if (markdownView && markdownView.file) {
				if (!checking) {
					handleSubMetadataCommand(markdownView.file, plugin, settings, entityStore);
				}
				return true;
			}
			return false;
	...
	plugin.addCommand({
		...
		checkCallback: (checking: boolean) => {
			...
			if (markdownView && markdownView.file) {
				if (!checking) {
					handleTimelineCommandExecution(markdownView.file, plugin, settings, timelineManager);
				}
				return true;
			}
			return false;
